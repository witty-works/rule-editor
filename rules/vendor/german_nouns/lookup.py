"""Vendored noun lookup from the `german-nouns` package.

See README.md in this directory for provenance, licensing and the list of
local modifications.
"""
...
from rules.vendor.german_nouns.config import CSV_FILE_PATH, INDEX_FILE_PATH

Record = Dict[str, Any]

# nouns.csv has rows far wider than the default field-size limit on some
# platforms; raise it before parsing.
...
class Nouns(object):
    def __init__(self) -> None:
        # parse csv file
        with gzip.open(CSV_FILE_PATH, "rt", encoding="utf-8", newline="") as f:
    ...
    def create_index(self) -> None:
        ...
        try:
            with gzip.open(INDEX_FILE_PATH, "wt", encoding="utf-8") as f:
                f.write(output)
        except OSError:
            # Read-only filesystem (e.g. the Platform.sh app container). The
            # in-memory index above is complete, so carry on without caching.
            pass

    def load_index(self) -> None:
        with gzip.open(INDEX_FILE_PATH, "rt", encoding="utf8") as f:
